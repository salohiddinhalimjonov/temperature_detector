...
from django.core.mail import EmailMultiAlternatives#A version of EmailMessage that makes it easy to send multipart/alternative messages.
...
from django.template.loader import render_to_string#loads a template like get_template() and calls its render() method immediately.
...
class RegisterUser(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            email = form.cleaned_data.get('email')#form.cleaned_data returns a dictionary of validated form input fields and their values,
            ...
            if smth is not None:
                login(request, smth)#To log a user in, from a view, use login(). It takes an HttpRequest object and a User object. login() saves the user’s ID in the session,
...
class PasswordResetRequest(View):
    def get(self,request):
        domain = request.headers['Host']#request.headers : A case insensitive, dict-like object that provides access to all HTTP-prefixed headers
        password_reset_form = PasswordResetForm(request.GET)
        return render(request, 'registration/reset_password.html', {'domain':domain, 'password_reset_form':password_reset_form})
    def post(self,request):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if search is not None:
                smth = AboutUrl.objects.filter(Q(title__contains=search) | Q(original_url=search), user=request.user).order_by('-pk')
...
class Create(View):
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            ...
            about_url = AboutUrl.objects.create(user=request.user, original_url=data['original_url'], title=data['title'], description=data['description'], date_published=datetime.now(), choices=data['choices'])
            about_url.save()
            return redirect('home')
...
class Update(View):
    ...
    def post(self,request,pk):
        ...
        new_form = UrlForm(request.POST or None, instance=new_info)#UrlForm(request.POST or None) : I forgot what is happening here
